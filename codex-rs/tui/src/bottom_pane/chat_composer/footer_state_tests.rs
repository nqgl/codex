...
#[test]
fn dictation_preview_keeps_draft_and_cursor_in_short_composers() {
    let mut composer = composer();
    composer.insert_str("editable draft");
    composer.set_dictation_preview("completed words", "partial words");
    ...
    for height in 4..=8 {
        ...
        let mut buffer = Buffer::empty(area);
        composer.render_with_options(area, &mut buffer, /*mask_char*/ None, options);
        assert!(text(&buffer).contains("editable draft"));
