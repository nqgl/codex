...
//! Live dictation uses its own bounded preview rows, separate from editable draft text.
...
impl ChatComposer {
    pub(super) fn layout_with_options(
        ...
        let dictation_preview = Rect {
            height: preview_height,
            ..textarea_rect
        };
        let preview_rows = preview_height.saturating_add(u16::from(preview_height > 0));
        textarea_rect.y = textarea_rect.y.saturating_add(preview_rows);
        textarea_rect.height = textarea_rect.height.saturating_sub(preview_rows);
        ComposerLayout {
            dictation_preview,
            status,
    ...
    pub(crate) fn desired_height_with_options(
        ...
        let remote_images_height: u16 = self
            ...
            .unwrap_or(u16::MAX);
        let remote_images_separator = u16::from(remote_images_height > 0);
        let preview_height = self.dictation_preview.desired_height(inner_width);
        self.draft.textarea.desired_height(inner_width)
            + preview_height
            + u16::from(preview_height > 0)
