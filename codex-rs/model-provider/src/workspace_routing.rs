...
impl WorkspaceRoutingContext {
    ...
    /// Resolve an account-backed service using the same workspace policy as Responses.
    /// Discovery errors propagate before credentials or content reach that service.
    #[expect(
        clippy::await_holding_invalid_type,
        reason = "serialize workspace discovery"
    )]
    pub async fn route_provider(
        ...
        auth: &CodexAuth,
    ) -> io::Result<ClientRedirectPolicy> {
        let mut previously_routed = self.previously_routed.lock().await;
        if let Some(routing) = auth_manager
            ...
            .await?
        {
            apply_workspace_routing(provider, routing)?;
            *previously_routed = true;
            return Ok(ClientRedirectPolicy::Reject);
        }
        Ok(ClientRedirectPolicy::Default)
    }
}
