...
- Within each bounded stage, run independent, non-conflicting calls concurrently. Use `Promise.allSettled` when partial results are useful and inspect every outcome; keep dependent calls and conflicting mutations sequential. Bound fanout and combined output.
- `bundles.create(value)` stores a bounded, session-scoped snapshot without adding its contents to model context and returns a Bundle; `bundles.open(id)` reopens it.
- Bundle helpers: `select(...names)`, `omitted()`, `info()`, `read(item, range)`, `search(query)`. After selecting one item, `read(range)` infers its name. Ranges accept `{startLine,lineCount}`, `{startChar,charCount}`, or `{chunk,chunkCount}`.
- `ask(question)` and `summarize(instructions?)` use read-only `gpt-5.6-terra` medium queries and return model/usage/freshness metadata. `each(...names).ask(...)` and `.summarize(...)` return another Bundle. Snapshots are immutable and may be stale; `store` and `load` preserve Bundle values."#;
...
pub fn build_exec_tool_description(
    ...
    if !description.is_empty() {
        let needs_runtime_guidance =
            !description.trim().is_empty() && !description.contains("bundles.create");
        sections.push(description);
        if needs_runtime_guidance {
            sections.push(CUSTOM_RUNTIME_GUIDANCE.to_string());
        }
    }
