        ...
        static CONFIG: LazyLock<BTreeMap<String, AgentRoleConfig>> = LazyLock::new(|| {
            BTreeMap::from([
                ...
                (
                    "explorer".to_string(),
                    AgentRoleConfig {
                        description: Some(r#"Use `explorer` for specific, well-scoped questions about the codebase. Explorers are fast and authoritative.
- Explorer answers are generally reliable enough to build on without re-exploring the same ground — though inspecting code yourself for context is always fine.
- Distinct independent questions can go to multiple explorers in parallel, and you can keep working on non-dependent tasks while they answer; that parallelism is most of delegation's advantage.
- Reuse an existing explorer for related follow-up questions."#.to_string()),
                ...
                (
                    "worker".to_string(),
                    AgentRoleConfig {
                        description: Some(r#"Use for execution and production work: implementing part of a feature, fixing tests or bugs, taking an independent chunk of a large refactor.
- Assign each worker clear ownership — which files or modules it is responsible for ("Worker 1 owns the authentication module, Worker 2 the database layer") — so parallel work doesn't collide.
- Tell workers they are not alone in the codebase: others' edits get accommodated, not reverted."#.to_string()),
                        config_file: None,
                        nickname_candidates: None,
                    }
                ),
