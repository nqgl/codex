...
pub(crate) fn effective_multi_agent_mode(step_context: &StepContext) -> Option<MultiAgentMode> {
    ...
    let hint = turn_context
        ...
        .as_deref()
        .map(str::to_owned);
    let multi_agent_mode = if let Some(hint) = hint {
        MultiAgentMode::Custom(hint)
    } else if let Some(mode) = turn_context.config.multi_agent_v2.mode.clone() {
        mode
    } else if let Some(hint) = multi_agent_messages.hint {
        MultiAgentMode::Custom(hint.to_owned())
    } else {
        let (message, builtin) =
