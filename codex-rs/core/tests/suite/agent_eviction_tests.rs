...
async fn queued_mail_and_cancelled_eviction_keep_worker_ownership() -> Result<()> {
    ...
    .await;
    let (_initial_worker, initial_parent_completion) =
    ...
    .await;
    let (worker, _parent_completion) =
        mount_completed_worker(&server, QUEUED_TASK, "queued-mail").await;
    ...
    wait_for_parent_completion_wake(&test.codex, &initial_parent_completion).await?;
