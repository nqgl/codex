            ...
            sse(vec![
                ...
                match reviewer_response {
                    ...
                    | ReviewerResponse::CompactionError => ev_custom_tool_call(
                        "reviewer-inspect",
                        "exec",
                        // Keep the continuation larger than the window even with custom
                        // bundle instructions, while a fresh retry review still fits.
                        "text('inspection-output'.repeat(1200));",
                    ),
