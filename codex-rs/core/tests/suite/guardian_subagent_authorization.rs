...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
    ...
    // The custom harness wakes an idle parent when its worker completes. This is
    // separate from the user-authorization turn exercised below.
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        ...
        .respond_with(core_test_support::responses::sse_response(sse(vec![
            ev_completed("parent-worker-completion"),
        ])))
    ...
    .await;
    ThreadIdle::wait(&worker_thread).await;
        ...
        tokio::time::timeout(std::time::Duration::from_secs(/*secs*/ 5), async {
            loop {
                ...
                if history.items().any(|item| {
                    matches!(item, ResponseItem::Message { id: Some(id), .. }
                        if id.to_string() == "deployment-commentary")
                }) {
                    break;
                }
    ...
    // ResponseMock records requests before the extra matcher runs. Parent wake requests can
    // therefore be recorded by these mocks without being their matched review request.
    let mut worker_requests = worker_review_request
        .requests()
        .into_iter()
        .filter(|request| {
    ...
    let worker_request = worker_requests.remove(0);
    ...
    let mut guardian_requests = guardian_review
        .requests()
        .into_iter()
        .filter(|request| request.body_json()["client_metadata"]["x-openai-subagent"] == "guardian")
        .collect::<Vec<_>>();
    assert_eq!(guardian_requests.len(), 1);
    let guardian_request = guardian_requests.remove(0);
    let guardian_transcript = guardian_request.body_json().to_string();
    if matches!(root_context, RootContext::RetainedAtMessageLimit) {
