...
async fn board_is_shared_with_children_survives_resume_and_skips_idle_notices() -> anyhow::Result<()>
{
    ...
    // Completion wakes are independent of board notifications in this harness. Keep
    // them out of the ordered board-tool responses, including after an idle child turn.
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        ...
        .respond_with(responses::sse_response(sse(vec![ev_completed(
            "parent-wake",
        )])))
